...
.. currentmodule:: effective.operators

.. note:: This section assumes that the classes and functions
   from :mod:`effective.operators` that it uses are in the namespace.
...
and statistics is either equal to :data:`effective.algebra.boson` or
:data:`effective.algebra.fermion`. Then use it inside an operator::
...
Each argument to the function :func:`effective.operators.OpSum` should
be an operator defined as::

  op1 = Op(tens1(ind1, ind2, ...), field1(ind3, ind4, ...), ...)

The arguments of the function :func:`effective.operators.Op` are
...
:func:`effective.operators.D` in the following way::
